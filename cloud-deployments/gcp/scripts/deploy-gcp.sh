...
# HotGigs.ai GCP Deployment Script
# Automated deployment to GCP GKE with Terraform and Kubernetes
# Usage: ./deploy-gcp.sh [environment] [action] [options]
...
validate_prerequisites() {
    ...
    for tool in gcloud terraform kubectl docker jq; do
    ...
    if ! gcloud auth list --filter=status:ACTIVE --format="value(account)" | head -n1 &> /dev/null; then
        ...
        log_error "Please run 'gcloud auth login' and try again"
        exit 1
    fi
...
build_and_push_images() {
    ...
    # Set registry URL
    local registry_url="${DOCKER_REGISTRY:-gcr.io/$GCP_PROJECT_ID}"
...
terraform_init() {
    ...
    terraform init \
        -backend-config="bucket=hotgigs-terraform-state-$GCP_PROJECT_ID" \
        -backend-config="prefix=infrastructure/terraform.tfstate"
...
terraform_plan() {
    ...
    terraform plan \
        ...
        -var="gcp_zone=${GCP_ZONE:-us-central1-a}" \
...
configure_kubectl() {
    ...
    local gcp_region=$(terraform output -raw gcp_region)
...
deploy_kubernetes_manifests() {
    ...
    local storage_bucket=$(terraform output -raw storage_bucket_name)
    local registry_url="gcr.io/$GCP_PROJECT_ID"
    ...
    # Deploy application
    export BACKEND_IMAGE="$registry_url/hotgigs/backend:$image_tag"
...
full_deployment() {
    ...
    send_notification "good" "HotGigs.ai deployed successfully to GCP $ENVIRONMENT (commit: ${commit_sha:0:8})"
}
...
handle_error() {
    ...
    local commit_sha=$(get_git_commit)
    send_notification "error" "HotGigs.ai GCP deployment failed in $ENVIRONMENT (commit: ${commit_sha:0:8})"
...
# Main execution
main() {
    log_info "Starting HotGigs.ai GCP deployment"
    log_info "Environment: $ENVIRONMENT"
    log_info "Action: $ACTION"
    log_info "GCP Project: ${GCP_PROJECT_ID:-not set}"
    log_info "GCP Region: ${GCP_REGION:-us-central1}"
